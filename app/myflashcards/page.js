...
export default function MyFlashcards() {
  ...
  return loading ? (
    ...
    <Container sx={{ marginLeft: 0 }} maxWidth="md">
      <h1 style={{ marginTop: "20px", textAlign: "center", width: "96vw" }}>
        My FlashCards
      </h1>
      {flashcardsSetList?.length > 0 ? (
        <div className="myflashcards-main">
          <div className="flashcards-list">
            {flashcardsSetList?.map((f, i) => {
              return (
                <div
                  onClick={async () => await selectFlashcardset(f?.name)}
                  key={i}
                  className={
                    f?.name === selectedFlashcards?.name
                      ? "selected"
                      : "flashcard-item"
                  }
                >
          ...
          >
            {!(
              selectedFlashcards?.data && selectedFlashcards?.data.length > 0
            ) ? (
              <h3 style={{ textAlign: "center", width: "100%" }}>
                Select a set of flashcards to view
              </h3>
            ) : (
              <>
                <Typography
        ...
        <h3 style={{ textAlign: "center", marginTop: "200px", width: "94vw" }}>
